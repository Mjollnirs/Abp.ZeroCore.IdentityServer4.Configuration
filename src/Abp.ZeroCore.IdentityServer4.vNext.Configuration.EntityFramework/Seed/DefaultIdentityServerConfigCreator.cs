...
namespace Abp.IdentityServer4vNext.Seed
{
    public class DefaultIdentityServerConfigCreator
    {
        ...
        public void Create()
        {
            if (_context.ApiResources.Count() == 0)
                _context.ApiResources.AddRange(ObjectMapper.Map<IList<IdentityServer4vNext.Entities.ApiResource>>(new List<ApiResource>()
                {
            ...
            if (_context.IdentityResources.Count() == 0)
                _context.IdentityResources.AddRange(ObjectMapper.Map<List<IdentityServer4vNext.Entities.IdentityResource>>(new List<IdentityResource>()
                {
            ...
            if (_context.Clients.Count() == 0)
                _context.Clients.AddRange(ObjectMapper.Map<List<IdentityServer4vNext.Entities.Client>>(new List<Client>()
                {
                    new Client()
                    {
                        ClientId = "client",
